...
Server (PDS), materialising its records into columnar views, and authoring new
...
chapter shows you the records that the materialising chapter turns into Arrow
...
The materialising chapter writes Parquet files and reads them back with
...
The tutorial is a single guided path, not a catalogue. It does not enumerate the
